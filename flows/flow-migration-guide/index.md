...
- The **THIS WILL** panel describes the three guarantees of the migration:
  ...
  - Transfer schedules, triggers, and notifications to the new Conditional Flow.
...
- Read through the structural changes so you understand how legacy concepts (such as the **Continue on Failure** toggle) have been translated into Conditional Flow primitives -- see the [Conditional Flows limitations table](/flows/conditional-flows/#limitations) for the full mapping.

The preview is read-only. No changes are made to your project until you explicitly start the migration.

## Step 3: Run the migration

When you are ready, click **Migrate Now** in the top-right corner of the Migration Preview to start the migration. Keboola triggers a job that creates a new Conditional Flow from your existing Flow configuration, transfers schedules, triggers, and notifications to it, and disables the legacy Flow.
...
- Schedules, triggers, and notifications have been transferred automatically -- you do **not** need to copy them over.
...
{% include tip.html content="If something does not look right in the migrated Conditional Flow, you can disable or delete it and re-enable the legacy Flow to continue running it while you investigate -- otherwise both would react to the same schedules and triggers, which were transferred to the new Conditional Flow during migration. The Migration Preview can also be opened again at any time against the current state of the source Flow." %}
