...
The set of procedures includes one that can conditionally [ABORT](https://help.keboola.com/transformations/snowflake-plain/#aborting-transformation-execution) transformation processing. 
This is useful when you want the processing to stop following a failed test, **avoid spending unnecessary credits** and **prevent loading corrupted data** into your transformation outputs.
...
| ID | DESCRIPTION |
|---|---|
| TEST_COLUMN_NULL_OR_EMPTY | Tests for presence of Null (or empty string) values in given column in given table. Fails when there is more than 0 null values. |
| TEST_COLUMN_NULL | Tests for presence of Null values in given column in given table. Fails when there is more than 0 null values. |
...
| TEST_COLUMN_FOREIGN_REF | Tests whether each record’s reference key in table matched reference record in given reference table. Fails when there is more than 0 records with no match. |
| TEST_SUM_IN_GROUPS_EQUAL | Tests whether sum of values in given column in group (optional) in table is equal in two tables (groups in both tables can differ). Fails when sum differs by more than 0. |
| TEST_ANOMALY_NUMERIC | Tests anomalies in numeric columns based on formula (mean() - value) > 2.5 * std())| |In words - if value in column differs from “average” value by more than standard deviation, it’s anomaly. |
| TEST_COLUMN_DATA_TYPE | Tests whether data type of given column in given table is of expected data type |
| TEST_COLUMN_VALUE_DATA_TYPE | Tests whether values in given column in given table is of expected data type. Returns values which don’t match expected data type. Useful for tables without defined data types. |
...
| TEST_VALUE_NOT_CONTAIN | Opposite of test_value_cointains. Fails when value DOES contain provided string. |
| TEST_VALUE_BEGIN | Tests whether value in given column in given table begins with provided string. Fails when value doesn’t begin with provided string. |
| TEST_VALUE_NOT_BEGIN | Opposite of test_value_not_begins. Fails when value DOES begin with provided string. |
| TEST_VALUE_END | Tests whether value in given column in given table ends with provided string. Fails when value doesn’t end with provided string. |
| TEST_VALUE_NOT_ENDS | Opposite of test_value_not_ends. Fails when value DOES end with provided string. |
| TEST_VALUE_REGEXP | Tests whether value in given column in given table matches provided regular expression. Fails when value doesn’t match with provided regexp. |
| TEST_VALUE_NOT_REGEXP | Opposite of test_value_not_regexp. Fails when value DOES match with provided regexp. |
...
| TEST_TABLE_EMPTY | Test if table is empty (contains zero records). If there are zero records, test fails. |
...
| TEST_VALUE_GREATER_THAN | Tests that each value in given column in given table is greater than provided value. Fails when value is NOT greater than provided value. |
| TEST_VALUE_LOWER_THAN | Tests that each value in given column in given table is lower than provided value. Fails when value is NOT lower than provided value. |
