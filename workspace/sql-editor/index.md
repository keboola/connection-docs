...
> **Note:** When using the **Clean workspace before loading** option, all **tables** and **views** in the workspace schema are removed before loading new data. However, **stored procedures** and **user-defined functions (UDFs)** are not removed and will persist across loads. This is because Keboola does not create these objects, so it does not manage or remove them. If you have created any routines in the workspace, you are responsible for managing them manually.
