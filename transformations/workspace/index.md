...
### Read-Only Storage
In case input and output mapping are not fitting well your use case, read-only Storage offers a straightforward access to data, drastically reducing the execution time. 
...
When an SQL transformation starts, SQL code is being executed immediately. All tables from Storage are read-only accessible from within the transformation. 
Transformations are able to use a somewhat persistent workspace and write directly to defined tables in Storage.
...
When read-only access is enabled, the workspace will get automatic access to all data in Storage.
