...
description: How Keboola loads data into tables — primary keys and deduplication, incremental loads, the _timestamp column with native data types, and automatic and manual incremental processing.
---

How data lands in a [table](/storage/tables/) depends on two settings: whether the table has a **primary key**, and whether the load is **incremental**. This page explains both, plus **incremental processing** — letting components read only the rows that changed since their last run.
