...
Clients like Cursor and Claude normally manage the MCP server for you against Keboola's [hosted server](/ai/mcp-server/connect/). Run your own copy when you want **permissions narrower than your Keboola role** (your own Storage token and workspace schema), or when you're testing, developing, or wiring up a custom client. You can run it with Docker or with `uv`/`uvx`.
...
<!-- VERIFY(owner: Jordan/Matyáš): Docker and uvx invocations, and the localhost Streamable HTTP example, were carried over from the previous page and not executed during this split. -->
...
- `KBC_WORKSPACE_SCHEMA` — your project's [workspace](/tutorial/manipulate/workspace/) schema, used for SQL queries.
...
**BigQuery projects** also need `GOOGLE_APPLICATION_CREDENTIALS` pointing at the JSON key file of a Google Cloud service account with access to your BigQuery data, for example `GOOGLE_APPLICATION_CREDENTIALS="/path/to/your/credentials.json"`.
...
2. **Run the container.** For Snowflake projects:
   ...
   For BigQuery projects, mount the credentials file as well:
   ...
   `--rm` removes the container when it stops. Inside Docker the server listens on `stdio` by default, which suits clients that invoke and manage Docker commands themselves.
...
- **Streamable HTTP clients** — connect to the host and port you started, e.g. `http://localhost:8000/mcp?storage_token=XXX&workspace_schema=YYY`.
