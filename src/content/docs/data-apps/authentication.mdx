...
Only set this up if people must sign in with their **company accounts** — so that access follows your directory, and someone who leaves the company loses it. Everything else is cheaper: **Basic (Password)** shares one password and needs nothing outside Keboola, and **GitHub**, **GitLab** or **JumpCloud** restrict access by organization or team with far less setup. OIDC is the only method that needs admin rights in a console outside Keboola.
:::
