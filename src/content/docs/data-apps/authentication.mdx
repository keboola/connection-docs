...
`<url-prefix>-<app-id>.hub.<stack-host>` is the whole host shown in the **App URL** block on the app's configuration page: the URL prefix, a hyphen, and the App ID (for example `toy-store-sales-74016144`). An app created without a URL prefix drops the hyphenated part and uses `https://<app-id>.hub.<stack-host>/_proxy/callback`.

You don't have to assemble any of this by hand. The **Callback URL** field under **Authentication** shows the finished value with a copy button, for every method that needs one.
