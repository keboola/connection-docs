...
<Tabs syncKey="oidc-provider">
  <TabItem label="Google Cloud">
    ...
    **Set up the consent screen.** Google keeps it under **Google Auth Platform**; open it directly at [console.cloud.google.com/auth/overview](https://console.cloud.google.com/auth/overview) and pick your project.
    ...
    1. If the page shows **Get started**, the project has no consent screen yet. Click it and fill in the wizard: **App name** and **User support email**, the **Audience** (**Internal** or **External**, see above), a contact email, then **Create**. If you see **Branding**, **Audience**, and **Clients** in the left menu instead, the consent screen already exists; carry on.
    2. Open [**Branding**](https://console.cloud.google.com/auth/branding). Under **Authorized domains**, add `keboola.com`, the domain your App URL belongs to, and save. Without it, Google refuses the redirect URI in the next step. (Google shows **Authorized domains** on projects with an **External** audience; on an **Internal** one it may restrict domains for you — if you don't see the field, carry on to the next step.)
    {/* TODO(human-review, Nikita): open Branding on an Internal-audience project and confirm whether Authorized domains is shown. If it is always there, drop the parenthesis. guide-tester MAJOR 4, 2026-09-22. */}
    3. Check [**Audience**](https://console.cloud.google.com/auth/audience): **Internal** for your organization only, **External** for any Google account. Leave the **Test users** list and **Publish app** alone. For the sign-in scopes Keboola uses, Google applies neither: an External app lets any Google account in while still in Testing. Publishing only matters if you want your app name and logo shown on the consent screen, and then Google asks for branding details and a verification review.
  ...
  <TabItem label="Microsoft Entra ID">
    ...
    *Console steps last walked through on 2026-09-14; the Entra admin center changes often, so check the menu names as you go.*
  </TabItem>
  <TabItem label="Okta">
    ...
    *Console steps last walked through on 2026-09-14; check the menu names as you go.*
  </TabItem>
  <TabItem label="Auth0">
...
1. A new app: set its code source and click **Deploy App**; the short wizard asks for the backend size and an inactivity timeout (details: [Create an app manually](/data-apps/getting-started/#create-an-app-manually)). Just testing sign-in? A **Streamlit** app with a one-line inline script is the quickest thing to deploy. An app that's already running needs **Redeploy App** for the new authentication to take effect; a stopped one, **Start App**.
2. When the status turns **Active**, click **Open App**. Your provider asks you to sign in, then sends you into the app.
3. Test with the accounts that matter, each in a fresh private window: one that should get in, one that shouldn't. Once you're in, the app's own session cookie keeps you signed in, so a second try in the same window proves nothing.
...
2. Set the **Authorization callback URL** to: the app's callback URL — `https://<url-prefix>-<app-id>.hub.<stack-host>/_proxy/callback`, the whole host from the **App URL** block plus `/_proxy/callback` (e.g., `https://my-app-12345678.hub.north-europe.azure.keboola.com/_proxy/callback`).
...
2. Set the **Redirect URI** to: the app's callback URL — `https://<url-prefix>-<app-id>.hub.<stack-host>/_proxy/callback`, the whole host from the **App URL** block plus `/_proxy/callback` (e.g., `https://my-app-12345678.hub.north-europe.azure.keboola.com/_proxy/callback`).
...
3. Set the **Redirect URI** to: the app's callback URL — `https://<url-prefix>-<app-id>.hub.<stack-host>/_proxy/callback`, the whole host from the **App URL** block plus `/_proxy/callback` (e.g., `https://my-app-12345678.hub.north-europe.azure.keboola.com/_proxy/callback`).
...
`<url-prefix>-<app-id>.hub.<stack-host>` is the whole host shown in the **App URL** block on the app's configuration page: the URL prefix, a hyphen, and the App ID (
