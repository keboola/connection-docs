...
<Tabs syncKey="ai-client">
  ...
  <TabItem label="Claude Desktop">
    ...
    3. Start a new chat and paste the prompt from [Describe the app](#describe-the-app), without the words "using kbagent". The skill prefers the MCP route in Claude Desktop, described in [The MCP route](#the-mcp-route); if it also finds kbagent, it asks which one to use. Without a way to push to Git, it builds a Streamlit app instead.
  </TabItem>
  ...
  <TabItem label="Terminal">
    ...
    4. Commit your code and push it to `main`. When Git asks, use any username and the token as the password. The code has to follow the layout in the skill's [reference](https://github.com/keboola/ai-kit/blob/main/plugins/dataapp-developer/skills/dataapp-development/references/python-js-apps.md).

       ```bash
       git push <https-url> HEAD:main
       ```
...
Keboola-managed Git repository, deploy the app, check that it loads its data,
and give me its URL and its page in Keboola
(<Keboola URL>/admin/projects/<project-id>/data-apps/<config-id>).
```

Paste the prompt as it is; the agent fills in the page link itself. It reads the skill, finds the table and queries a sample before it writes any code. If kbagent knows more than one project, name the one to use. Name a framework too if it matters to you. To use your own GitHub repository instead, put its URL in the prompt; the repository then has to follow the layout in [Build an app locally](/data-apps/build-locally/).

If the agent can reach Keboola more than one way, it may ask which to use, or offer to sign you in to an MCP server. To follow this page, pick kbagent and decline that sign-in. The plugin's own MCP server is fixed to the US GCP stack (`us-east4`), so signing in to it points the agent at a project on that stack. [The MCP route](#the-mcp-route) describes the other way in.
...
{/* VERIFY(Nikita): the revised prompt's "check that it loads its data" has not been run yet; on 2026-09-23 the agent found the missing workspace by reading the logs on its own. */}
