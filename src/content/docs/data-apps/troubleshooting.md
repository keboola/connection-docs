...
| Storage Access is on, but the app still has no `WORKSPACE_ID` | The app was created with `kbagent data-app create --use-managed-git-repo`, which leaves out the repository block Keboola needs to attach a workspace, and `kbagent data-app deploy` in 0.94.0 and earlier doesn't add it. | Add the block and deploy again, as in [If the app can't read Storage](/data-apps/build-with-an-agent/#if-the-app-cant-read-storage). |
