...
| Storage Access is on, but the app still has no `WORKSPACE_ID` | The app was created with `kbagent data-app create --use-managed-git-repo`, which leaves out the repository block Keboola needs to attach a workspace, and `kbagent data-app deploy` in 0.95.0 and earlier doesn't add it. | Add the block and deploy again, as in [If the app can't read Storage](/data-apps/build-with-an-agent/#if-the-app-cant-read-storage). |
| Every data load fails with `Query Service: Promise.withResolvers is not a function` | The app uses a newer `@keboola/api-client` than the app runtime's Node.js 20 can run. Version 39.0.0 calls `Promise.withResolvers`, which Node.js 20 doesn't have; the setup log shows the runtime's Node.js version. | Pin `@keboola/api-client` to the version in the `dataapp-developer` template, `^27.0.0`, then push and redeploy. |
| `git push` or `git clone` of a Keboola-managed repository fails with `Repository not found` | Git sent a stored login for that host, from its credential helper, instead of the app's token. | Run the command with the helper cleared, `git -c credential.helper= push <https-url> HEAD:main`, and give the token as the password when Git asks. See [Build an app with an AI agent](/data-apps/build-with-an-agent/). |
...
<!-- Promise.withResolvers row verified 2026-09-23 in project 264: the runtime's setup log says node v20.19.2; draft 74021870 with @keboola/api-client 39.0.0 failed every refresh, app 74021867 with 27.0.0 (the template's ^27.0.0) loaded its data. Both versions declare engines node 24.x. VERIFY(Michal Ševčík): whether the Python/JS runtime moves to Node.js 24 (AJDA-3318), which would retire this row. -->
