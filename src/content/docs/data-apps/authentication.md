...
Once an app is deployed, its URL is publicly available. Protect it so only the right people can open it, and choose the method that fits your audience.

![Authentication options](/data-apps/auth-options.png)

## Authentication methods

- **None** — the app is public to anyone with the URL. You can still add your own authorization inside the app; for Streamlit, use the [Streamlit authenticator](https://github.com/mkhorasani/Streamlit-Authenticator) ([example](https://github.com/KB-PS/mkt-bi-ocr/blob/master/Select_Invoices.py)).
- **Basic authentication** — a shared password generated by Keboola. Users enter it before the app opens. Quick for internal sharing. <!-- TODO(human-review, Miro): note the password-rotation gotcha flagged in Loop A. -->
- **OIDC (single sign-on)** — users sign in with your identity provider (Auth0, Google Cloud, Microsoft Entra ID, Okta). Recommended for anything beyond a quick share.
