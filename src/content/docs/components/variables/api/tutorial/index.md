...
First, store the [API endpoints](/overview/#apis) as environment variables, so we don't have to repeat ourselves.
...
The host names depend on your [stack](/overview/#apis):
...
A [Storage API Token](/management/project/tokens/) is needed to interact with the [Keboola APIs](/overview/#apis).
