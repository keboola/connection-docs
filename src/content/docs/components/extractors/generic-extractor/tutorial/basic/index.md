...
[JSON format](/components/extractors/generic-extractor/tutorial/json/). This tutorial uses the
[MailChimp API](https://mailchimp.com/developer/reference/), so
have its documentation at hand. You also need the
[MailChimp API key](/components/extractors/generic-extractor/tutorial/#prepare).
...
- `Incremental Output` option, which defines whether you want the result to overwrite the existing data or append to it. [See more](/components/extractors/generic-extractor/incremental/)
  - Note that when using Incremental Output, you should set up the mapping.

![Base Configuration](/components/extractors/generic-extractor/tutorial/base_configuration.png)
...
The `password` property is prefixed with the hash mark `#`, meaning the value will be [encrypted](/extend/encryption/) once
...
![New Endpoint](/components/extractors/generic-extractor/tutorial/new_endpoint.png)

You will be asked to provide the relative endpoint URL path. In our case, we will use the `campaigns` endpoint.

![New Endpoint modal](/components/extractors/generic-extractor/tutorial/new_endpoint_modal.png)
...
![Test endpoint](/components/extractors/generic-extractor/tutorial/test_endpoint.png)
...
![Selector](/components/extractors/generic-extractor/tutorial/data_selector.png)

Now, run the configuration by clicking the **Run** button and go to the job details to see what happened:

![Screenshot - Generic Extractor job](/components/extractors/generic-extractor/tutorial/job-1.png)
...
![Screenshot - Campaigns Table](/components/extractors/generic-extractor/tutorial/table-campaigns-sample.png)
