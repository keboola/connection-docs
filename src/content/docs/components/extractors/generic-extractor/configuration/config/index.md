...
from within [functions](/components/extractors/generic-extractor/functions/).

The keys prefixed by the hash character `#` are [automatically encrypted](/extend/common-interface/encryption/) when the
...
must always refer to it as `#password` (for instance, in [functions](/components/extractors/generic-extractor/functions)).
...
(the names of the tables are defined by the [`dataType`](/components/extractors/generic-extractor/configuration/config/jobs/#dataType) setting):
...
is described in a [separate article](/components/extractors/generic-extractor/configuration/config/mappings/).
...
Read more about running Generic Extractor in a [separate article](/components/extractors/generic-extractor/running/).

## HTTP
The `http` option allows you to set the HTTP headers sent with every request. This primarily serves the purpose of providing values for [`api.http.requiredHeaders` option](/components/extractors/generic-extractor/configuration/api/#required-headers).
It is also possible to use the `http` option without `api.http.requiredHeaders` in
which case it is essentially equal to [`api.http.headers`](/components/extractors/generic-extractor/configuration/api/#default-headers).
...
is described in a [dedicated article](/components/extractors/generic-extractor/incremental/).
...
[functions](/components/extractors/generic-extractor/functions/) as `userData` property values.
...
### Level 2
Level 2 has different behavior in [`responseFilter`](/components/extractors/generic-extractor/configuration/config/jobs/#response-filter) handling.
