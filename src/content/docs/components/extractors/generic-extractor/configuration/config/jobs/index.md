---
title: Jobs
description: 'Define what Generic Extractor requests — endpoint, HTTP method, request parameters, data type and data field, response filters, and how responses are merged.'
...
**single job represents a single [API resource](/components/extractors/generic-extractor/tutorial/rest)**.
...
[`responseFilter` setting](/components/extractors/generic-extractor/configuration/config/jobs/#response-filter).
...
API). It is also allowed to use [function calls](/components/extractors/generic-extractor/functions/).
...
See our [examples](/components/extractors/generic-extractor/configuration/config/jobs/#examples-with-http-methods-and-parameters).

## Method
The `method` parameter defines the [HTTP request method](/components/extractors/generic-extractor/tutorial/rest/).
...
With the POST method, also [pagination](/components/extractors/generic-extractor/configuration/api/pagination/) parameters are sent in the request body, and **not as URL query parameters**. If your API requires pagination parameters in the query, use the GET method or Custom Python component instead.
...
Data types are used in [mappings](/components/extractors/generic-extractor/configuration/config/mappings/) and for naming output
tables within their [output buckets](/components/extractors/generic-extractor/configuration/config/#output-bucket).

Note that you can use the same `dataType` for multiple resources, provided that the result objects may
be [merged into a single one](/components/extractors/generic-extractor/configuration/config/mappings/). This can be used,
...
- If the response is an [object](/components/extractors/generic-extractor/tutorial/json/) and there is a single *array* property,
use that property.
...
[JSON Parser](/components/extractors/generic-extractor/configuration/config/jobs/#merging-responses) (as in the above examples).
...
[separate article](/components/extractors/generic-extractor/configuration/config/jobs/children/).

## Scroller

The `scroller` parameter assigns a predefined scroller when
[`multiple` pagination](/components/extractors/generic-extractor/configuration/api/pagination/multiple/) is used,
and is pointless when the `multiple` pagination method is not used.

If `scroller` is not set, the pagination method specified in the [`api` configuration](/components/extractors/generic-extractor/configuration/api/pagination/)
...
Notice that the [boolean value](/components/extractors/generic-extractor/tutorial/json/#data-values) `married` is converted
...
[Boolean value](/components/extractors/generic-extractor/tutorial/json/#data-values)
...
[GET parameter](/components/extractors/generic-extractor/tutorial/rest/#url) `type` to specify which
...
The [`params` configuration](/components/extractors/generic-extractor/configuration/config/jobs/#request-parameters) option specifies the
...
[functions](/components/extractors/generic-extractor/functions/).

### POST request
You may encounter an API which is not exactly [RESTful](/components/extractors/generic-extractor/tutorial/rest/)
and has to be queried using the [HTTP POST method](/components/extractors/generic-extractor/tutorial/rest/#method).
...
Generic Extractor can handle this too, using the [`method` configuration](/components/extractors/generic-extractor/configuration/config/jobs/#method):
...
A not-exactly-[RESTful](/components/extractors/generic-extractor/tutorial/rest/) API (see above) may require some JSON
...
[`params` configuration](/components/extractors/generic-extractor/configuration/config/jobs/#request-parameters).
...
In the HTTP protocol, this would be encoded in the following [query string](/components/extractors/generic-extractor/tutorial/rest/#url):
