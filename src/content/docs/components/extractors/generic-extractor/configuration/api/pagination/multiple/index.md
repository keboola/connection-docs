...
Each [job](/components/extractors/generic-extractor/configuration/config/jobs/) is then assigned a 
...
[paging strategies](/components/extractors/generic-extractor/configuration/api/pagination/#paging-strategy) can be used, and 
multiple paging strategies can be mixed. The configurations are the same as if there was a single scroller.
The name of the scroller must be used in a specific [job `scroller` parameter](/components/extractors/generic-extractor/configuration/config/jobs/#scroller).
