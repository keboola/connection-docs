...
- [`response.url`](/components/extractors/generic-extractor/configuration/api/pagination/response-url/) --- uses a URL provided in the response.
- [`offset`](/components/extractors/generic-extractor/configuration/api/pagination/offset/) --- uses the page size (limit) and **item offset** (like in SQL).
- [`pagenum`](/components/extractors/generic-extractor/configuration/api/pagination/pagenum/) --- uses the page size (limit) and **page number**.
- [`response.param`](/components/extractors/generic-extractor/configuration/api/pagination/response-param/) --- uses a specific value (token) provided in the response.
...
[`response.param` method](/components/extractors/generic-extractor/configuration/api/pagination/response-param/).
...
- Use the [`pagenum` method](/components/extractors/generic-extractor/configuration/api/pagination/pagenum/) if the API expects the **page**
number/index. For example, `/users?page=2` retrieves the 2nd page regardless of how many items the page contains.
- Use the [`offset` method](/components/extractors/generic-extractor/configuration/api/pagination/offset/) if the API expects the **item**
number/index. For example, `/users?startWith=20` retrieves the 20th and following items.
- Use the [`cursor` method](/components/extractors/generic-extractor/configuration/api/pagination/cursor/) if the API expects an item **identifier**.
...
- **`field`** (required) --- name of a field containing any value. The field must be in the root of the response.
    It will be converted to [boolean](/components/extractors/generic-extractor/tutorial/json/#data-values).
