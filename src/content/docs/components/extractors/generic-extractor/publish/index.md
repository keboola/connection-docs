...
The values obtained from the end user will be stored in the [`config` property](/components/extractors/generic-extractor/configuration/config/).
...
negotiate [OAuth authentication](/components/extractors/generic-extractor/configuration/api/authentication/#oauth).
An example of the template UI is shown in the picture below.

![Screenshot - Generic templates UI](/components/extractors/generic-extractor/template-1.png)
...
Notice that the JSON editor allows modification only to the [`config`](/components/extractors/generic-extractor/configuration/config/)
section. Other sections, such as [`api`](/components/extractors/generic-extractor/configuration/api/) or
[`authorization.oauth_api`](/components/extractors/generic-extractor/configuration/api/authentication/#oauth), may not be modified by the end user.
...
![Screenshot - Schema Test](/components/extractors/generic-extractor/schema-test.png)
...
The above properties will be merged into the [`config` section](/components/extractors/generic-extractor/configuration/config/). Now
modify the configuration so that it reads them from there using [functions and references](/components/extractors/generic-extractor/functions/).
...
[`concat` function](/components/extractors/generic-extractor/functions/#concat), which joins together the
