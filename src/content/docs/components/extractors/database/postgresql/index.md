...
:::caution
The smallest resumable unit is a table. If the snapshot process fails while fetching a specific table, its partial result is stored. On the next job execution, the snapshot for that table restarts. <br>In Append Mode, this may result in duplicate rows, which the consumer must handle appropriately.
:::
...
:::caution
The <code>REPLICA IDENTITY</code> setting determines what is included in the <code>UPDATE</code> and <code>DELETE</code> events. If it is not set to <code>FULL</code>, these events will include only the primary key values or may not be emitted at all if no primary key is present.
:::
...
:::caution
For the connector to detect and process events from a heartbeat table, you must add the table to the PostgreSQL publication used by the connector.
:::
...
You may opt to use an SSH Tunnel to secure your connection. The [developer documentation](/components/extractors/database/ provides detailed instructions for setting up an SSH tunnel.
