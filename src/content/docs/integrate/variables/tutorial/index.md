...
The result will be the parametrized configuration of the [Generic Extractor](https://developers.keboola.com/extend/generic-extractor),
...
First, store the [API endpoints](https://developers.keboola.com/overview/api/) as environment variables, so we don't have to repeat ourselves.

We will need:
- [Storage API](/integrate/storage/api/) to store the variable definitions and the extractor configuration -
- [Job Queue API](https://developers.keboola.com/extend/job-queue/) to run the extractor job from the configuration.

The host names depend on your [stack](https://developers.keboola.com/overview/api/#stacks-and-endpoints):
...
A [Storage API Token](/management/project/tokens/) is needed to interact with the [Keboola APIs](https://developers.keboola.com/overview/api/#list-of-keboola-apis).
