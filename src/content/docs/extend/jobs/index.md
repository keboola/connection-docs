...
[Components](/components/) differ in their upper limits on how long a job can be executing and how much memory it is allowed to consume.
...
![Job State transitions](/extend/jobs/states.png)
...
When you create a job it is in the `created` state. In a success scenario it will transition to a `processing` state and when the actual work is done, to the
 ...
 - Orchestration [phases](/flows/flows-legacy/#execute-tasks-in-parallel). When you run an orchestration, the jobs for all phases are created. Phases that depend on other phases enter the `waiting` state.
...
The `container` job represent a job containing [parallel executions](#job-runtime-configuration) 
of configuration rows. `phaseContainer` type contains standard jobs in a single 
phase of an orchestration. `orchestrationContainer` job type represents an [orchestration](/flows/flows-legacy/) and 
contains phase jobs of that orchestration. What these job types have in common is a strong 
[parent-child relationship](#job-id). This means for example that when a child job fails, the container fails too. The 
...
[Job status polling](https://en.wikipedia.org/wiki/Polling_%28computer_science%29).
