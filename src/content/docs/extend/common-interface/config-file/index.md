...
data, use [encryption](/extend/encryption/). This section is not available in Transformations.
...
provide different values for different [Keboola Stacks](/overview/api/#stacks-and-endpoints). Values in
*Stack Parameters* are merged with those in *Image Parameters* with *Stack Parameters* having a higher priority.
*Stack Parameters* are indexed with [Storage URL](/overview/api/#stacks-and-endpoints) or the given region.
...
#### Encryption
Both *Image Parameters* and *Stack Parameters* support [encrypted values](/extend/encryption/). In practice, however,
...
Also take care to use the correct [API URL](/overview/api/#stacks-and-endpoints) to obtain
...
[Encryption](/extend/encryption/#encrypting-data-with-api) is applied to the state the same way it is applied to
