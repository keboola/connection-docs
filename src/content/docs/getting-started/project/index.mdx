...
**Where you are:** nothing exists yet. **When you finish:** a project you can sign in to, and
you know where its Storage, Jobs and Components live.

**No Kai path here:** Kai works *inside* a project, so this one page is yours. Every page after
it can be done either way.

{/* Tutorial-type page. Ordinal removed 2026-09-02 (see DECISIONS.md). Converted .md -> .mdx the
same day so the shared cold-start components can be imported. The project-screen tour below was
verified live in
...
<Prereqs needs={[]}>
  <li>An email address — nothing else.</li>
  <li>
...
to be a project first. From here on, the building pages offer both paths as a
**Do it with Kai** tab — on [Send a table to Google Sheets](/getting-started/write/) it takes
over only after you have authorized Google yourself.
...
Two things. The second one is worth settling now rather than halfway through
...
- **Kai Agent** in the top bar opens a chat panel when you click it. Worth confirming now,
  ...
  click-through path in full. The two pages that assume Kai are
  [Ask a question and get an answer](/getting-started/ask/) — where you can run the same
  aggregate in a [workspace](/getting-started/transform/workspace/) instead — and
  [Put it in front of someone](/getting-started/app/), whose manual route is a development
  workflow in the Apps section.
