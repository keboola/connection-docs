...
one 10,000-row atlas that turns IDs into names and places every sighting in its ocean and depth
zone, and it introduces the mapping model that keeps your source data safe while you do it.
Step 3 of the [Getting Started](/getting-started/) arc.

**Two ways to do it** — pick a tab in *Set it up* below. Doing it with Kai, the run is part of
the prompt — you only check the result. Doing it yourself, you run it afterwards in the section
below the tabs.

{/* Tutorial-type page (step 3 of 7). UI labels walked live in project 264 on 2026-08-04: the
...
Four tables in Storage — `occurrences`, `species`, `depth_zones` and `basins` — from
...
answers things the raw file could not — sort by `depth_m` and the deepest row is a
*Grimpoteuthis challengeri*, a dumbo octopus, recorded at **4,838 meters** in the abyssal zone.
