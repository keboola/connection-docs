...
you get to read the joins before they run. One difference from the hand-built version: Kai builds
the transformation on **read-only input**, so you will not see an input mapping in its
configuration — the mapping sections on this page describe the hand-built path.

**Check:** 639 rows, 23 columns — the original 15 plus the eight named in
[Run it and check the result](#run-it-and-check-the-result), below the tabs. If the count differs,
the columns differ: re-prompt, or fix the code in place. Later steps only depend on the table name, so a close-enough table is fine.
