...
{/* VERIFY(owner): the read-only-input behaviour is Jordan's correction from the 5 Aug review and
is repeated here as fact, but nothing under kai/ documents it and it has not been re-walked since
the dataset changed. It is load-bearing: read-only input needs bucket-ID-qualified table names
(workspace/index.md:285), which the prompt above does not use. Re-walk the Kai tab in 264 and
record the result, or qualify both. */}
