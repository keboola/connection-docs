...
Data arrives split across files that cannot answer a question on their own. Here it is four:
the readings themselves, plus three small lookup tables that spell out what the readings' codes
mean. Apart, the readings file can plot 10,000 dots but cannot say what any dot *is*. This page
gets all four into Storage, so the next page can join them into one table that can.

**Where you are:** you have a project, and Storage is empty. **When you finish:** four tables in
Storage and a connector configuration you can re-run.

<Prereqs needs={['project', 'nothing']} />

<PathIntro yours="creating the project itself" />

{/* Tutorial-type page. Ordinal removed 2026-09-02: page order lives in the sidebar, not in
the copy (see DECISIONS.md). Rebuilt on the HTTP connector: CSV Import is deprecated and cannot
take new configurations (verified live in project 264, 2026-08-04). Every label, default and
dialog below was walked live in project 264 on 2026-08-04 with keboola.ex-http; the run was
re-walked on the current sample on 2026-08-25 and the captures reshot 2026-08-27. Note our own
connector page (components/extractors/storage/http/index.md) still says "Add Table"; the UI says
"Add Row" — and its lede still claims the connector "loads a single CSV file", which this page's
four-row configuration contradicts. */}
...
flow, which is what [Run it on a schedule](/getting-started/automate/) depends on.
...
One configuration with four rows, not four configurations: [the flow](/getting-started/automate/)
...
- [Load from Google Sheets](/getting-started/load/googlesheets/) — a small reference table pulled
