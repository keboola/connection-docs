...
questions that only read do not ask. If confirming each one gets tiring, turn on **plan mode**
with the button in the chat's message box: Kai drafts the whole plan and you approve it once,
instead of approving every step. Failing that, **Always allow** and
[tool permissions](/kai/settings/#tool-permissions) do the same job more bluntly.
{/* Plan mode confirmed live 2026-09-02: the composer carries a button labelled "Enable plan
mode" (aria-label), alongside "Disable follow mode". The earlier VERIFY asked for the name — this
is it. Still owed: a description on the kai/ pages, which document neither mode. */}
