...
<Prereqs needs={['project', 'kai']}>
  ...
  <li>
    <strong>A Google account</strong> only if you want the spreadsheet side trip — that consent
    screen is in your own account.
  </li>
</Prereqs>

Nothing needs installing. Everything happens in the browser, and the whole build needs about
forty-five minutes of your attention.
...
dialog and [tool permissions](/kai/settings/#tool-permissions) pre-approve a tool for the rest
of the chat or the project. The chat's **plan mode** button is a different thing: Kai writes the
steps down and asks you to approve the plan before it starts — and then still asks for each
change. Use it when you want to read the plan first.
...
2. **[Get your data in](/getting-started/load/)** — pull the sample files into Storage with a
   connector, and understand buckets, tables and stages. One Kai prompt, run included.
3. **[Transform data](/getting-started/transform/)** — the SQL that turns codes into names and
   places every reading in context. One Kai prompt builds and runs it.
4. **[Ask a question and get an answer](/getting-started/ask/)** — put the guide's question to
   your own data and have Kai answer it out of the table you just built.
5. **[Put it in front of someone](/getting-started/app/)** — a map with a "how close to me?"
   field, described to Kai in one sentence and published with one button.
6. **[Make it run every morning](/getting-started/automate/)** — run the whole thing in order,
   on a schedule, with notifications. Kai wires the flow; the schedule is two clicks.
7. **[Check it worked](/getting-started/check/)** — what should be in Storage and Jobs, the one
   number that proves the join, and the failures that actually happen.
...
- **[Send a table to Google Sheets](/getting-started/write/)** — deliver the result to where
  people actually read it. Needs a Google account you can authorize.
- **[Load from Google Sheets](/getting-started/load/googlesheets/)** and
  **[Load from a database](/getting-started/load/database/)** — load from a source that needs
  credentials, rather than the public URL the guide uses.
