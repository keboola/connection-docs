...
{/* kbagent-check: skip-next. The vendored reference is v0.76.1, which predates the `auth` group; the v0.91.0 bump that adds it is PR #1044. Drop this skip and the one below once that merges. */}
```bash
...
{/* kbagent-check: skip-next. See the note above. */}
```bash
