...
The **Orchestration Trigger** application triggers Keboola [Flows](/flows/) (both standard and conditional) — typically across different Keboola projects in a [Multi-Project Architecture](/catalog/multi-project/) setup.
...
* A [Limited Access Token](/management/project/tokens/#limited-access-to-components) with access to the Flow and Conditional Flow components, every component used inside the flow, and the buckets those components interact with.
...
* **Flow ID** (required) – The configuration ID of the Flow or Conditional Flow to trigger. Use the **RE-LOAD FLOWS** button to populate the picker with flows from the destination project; both Conditional Flows and Flows are listed.
...
* **Fail on Warning** – Visible only when **Wait for job finish and check jobs status** is enabled. When checked, the trigger job fails if the triggered flow ends with a warning.
...
* **Flow on failure settings** – Visible when **Trigger flow on failure** is enabled. Contains:
  ...
  * **Flow ID** (required) – The configuration ID of the recovery Flow or Conditional Flow. Use **RE-LOAD FLOWS** to populate the list; refresh after changing **Flow Location**.
