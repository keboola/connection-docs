...
| ~~Hero clip for the hub~~ | Done 2026-09-14 as `outlook-tour.mp4` (three acts, poster = the question); replaced 2026-09-23 by `hub-demo.mp4`, re-cut bright on 2026-09-25 after Nikita found it dark (27.1 s, 579 KB, luma 223; five beats: two Kai answers, the app prompt, the Apps list, the grid with the red figures outlined, then a closing line), poster = Kai's one-line answer. Still owed: a taller grid capture, because `app/grid-preview.png` slices the Brno row; it needs the app running and unlocked, which the broken deploy prevents. Two of Jordan's 21 Aug clip decisions changed with it, VERIFY(Jordan) in DECISIONS.md. | — |
